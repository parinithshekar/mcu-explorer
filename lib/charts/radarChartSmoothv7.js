...
function RadarChart(id, data, options) {
  ...
  // // Custom tooltip to show character information on blob
  const blobTooltip = d3
    .select('#root')
    .append('div')
    .style('opacity', 0)
  ...
  blobWrapper
    ...
    .attr('blob-name', function (d, i) {
      return cfg.blobNames[i];
    })
    ...
    .on('mousemove', function (d, i) {
      ...
      blobTooltip
        .html(d3.select(this).attr('blob-name'))
        .style('left', d.pageX + 10 + 'px')
        .style('top', d.pageY - 60 + 'px');
    })
    .on('mouseout', function () {
      // Disable tooltip
      blobTooltip.html('').style('left', 0).style('top', 0).style('opacity', 0);
  ...
  blobCircleWrapper
    ...
    .on('mouseout', function () {
      tooltip.transition().duration(200).text('').style('opacity', 0);
    });
