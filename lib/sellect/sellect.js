...
(function () {
  ...
  function createHTML() {
    ...
    if (self.options) {
      self.options.originList.forEach(function (item) {
        createListsHTML(self.originListHTML, item, self.options.type);
      });

      self.options.destinationList.forEach(function (item) {
        createListsHTML(self.destinationListHTML, item, self.options.type);
      });
  ...
  function createListsHTML(list, item, itemType) {
    var listItem = document.createElement('span');
    var listIcon = document.createElement('i');
    var colorSwatch = document.createElement('i');
    listItem.classList.add('sellect-trigger', 'sellect-item');
    listIcon.classList.add('fa', 'fa-times', 'sellect-close-icon');
    colorSwatch.classList.add('fas', 'fa-square', 'sellect-color-swatch');

    colorSwatch.style.color = (itemType==='character') ? CHARACTER[item].color : MOVIE[item].color;
    ...
    listItem.style.borderColor = (itemType==='character') ? CHARACTER[item].color : MOVIE[item].color
  ...
  function swapItemDOM(trigger, list) {
    ...
    // Added by @parinithshekar to avoid bug
    if (item.classList.contains('sellect-container')) {
      // Do nothing to avoid a bug
      return;
    }
  ...
  function filterOriginList() {
    ...
    items.forEach(function (item) {
      if (item.innerText.toLowerCase().indexOf(string.toLowerCase()) !== -1) {
  ...
  _Sellect.prototype.getSelected = function () {
    ...
    for (var i = 0; i < self.destinationListHTML.childNodes.length; i++) {
      self.options.destinationList.push(self.destinationListHTML.childNodes[i].textContent.trim());
    }
  ...
  _Sellect.prototype.getUnselected = function () {
    ...
    for (var i = 0; i < self.originListHTML.childNodes.length; i++) {
      self.options.originList.push(self.originListHTML.childNodes[i].textContent.trim());
    }
